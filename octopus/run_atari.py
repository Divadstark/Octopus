"""An Octopus agent training on Atari.
...
from dqn_zoo.octopus import agent
...
_SEED = flags.DEFINE_integer('seed', 1, '')
...
_RESULTS_CSV_PATH = flags.DEFINE_string(
    'results_csv_path', '/results/results_oct_increase_amidar_seed1.csv', ''
)
...
_NORMALIZE_WEIGHTS = flags.DEFINE_bool('normalize_weights', False, '')
...
_USE_MUNCHAUSEN = flags.DEFINE_bool('use_munchausen', True, 'Toggle for using the Munchausen term in the target calculation.')

def main(argv):
  del argv
  logging.info(
      'Octotpus on Atari on %s.', jax.lib.xla_bridge.get_backend().platform
  )
  ...
  train_agent = agent.Octopus(
      ...
      use_munchausen = _USE_MUNCHAUSEN.value,
  )
