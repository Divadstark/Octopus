"""A MDQN agent training on Atari.
...
from dqn_zoo.mdqn import agent
...
_RESULTS_CSV_PATH = flags.DEFINE_string(
    'results_csv_path', '/results/results_mdqn_amidar_seed1', ''
)
...
def main(argv):
    ...
    del argv
    logging.info('MDQN on Atari on %s.', jax.lib.xla_bridge.get_backend().platform)
    ...
    train_agent = agent.MDqn(
        ...
        tau=_TAU.value,
        alpha=_ALPHA.value,
        clip_value_min=_CLIP_VALUE_MIN.value,
    )
